...
{	
	...
	//load info
	var div = Q.createDOM("div", {innerHTML: "推荐使用分辨率≥1280*720显示器...<br>", style:
	{
		id: "loader",
		position: "absolute",
		width: this.width + "px",
...
{
	var content = "推荐使用分辨率≥1280*720显示器....<br>(" + Math.round(e.target.getLoadedSize()/e.target.getTotalSize()*100) + "%)";
...
game.showFPS = function()
{
	...
	if(fpsContainer)
	{
		...
		{
			fpsContainer.innerHTML = "返回游戏站<br>FPS:" + me.frames;
			me.frames = 0;
		}, 1000);
	}
};
